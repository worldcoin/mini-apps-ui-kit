...
import { fr, ar, type CountryLocaleCode } from "@worldcoin/mini-apps-ui-kit-react/countries";
...
You can also import locales from the main package entry:
...
All locales provided by `i18n-iso-countries` are supported. Import only what you need from `@worldcoin/mini-apps-ui-kit-react/countries` (preferred for tree‑shaking) or from the main package entry.

<br/>

### Types

The `CountryLocaleCode` type is available for type safety when working with locale codes:

```ts
import type { CountryLocaleCode } from "@worldcoin/mini-apps-ui-kit-react/countries";
