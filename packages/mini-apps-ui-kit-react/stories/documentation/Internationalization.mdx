...
export default async function Page({ searchParams }) {
  ...
  // Load locale data on the server using the SSR-friendly loader
...
`I18nProvider` seeds an isolated i18n store for its subtree, so each request gets its own locale registry without leaking data between renders.

If you need even more control (e.g., multiple stores in tests), create a store manually and pass it through the provider:

```tsx
import { createI18nStore, I18nProvider } from "@worldcoin/mini-apps-ui-kit-react";

const store = createI18nStore();
store.registerLocale(localeData);

<I18nProvider store={store}>{/* ... */}</I18nProvider>;
```
